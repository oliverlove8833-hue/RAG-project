...
class ParentDocumentRetriever:
    def __init__(self, client: QdrantClient, embeddings: OpenAIEmbeddings):
        ...
        self.child_collection = "노동자_근로기준_상담"
            ...
            for parent_id, chunks in parent_groups.items():
                ...
                parent_doc = Document(
                    ...
                    metadata={
                        ...
                        "parent_id": parent_id,
                        "category": first_chunk.metadata.get("category", "")
                    }
                )
...
class MetadataFilteredRetriever:
    def __init__(self, client: QdrantClient, embeddings: OpenAIEmbeddings):
        self.client = client
        self.embeddings = embeddings
        self.collection_name = "노동자_근로기준_상담"
    ...
    def search(self, query: str, k: int = 3, categories: List[str] = None) -> List[Document]:
        ...
        try:
            ...
            # 카테고리 필터로 결과가 없으면 필터 없이 재검색
            # (적재된 청크에 category 메타데이터가 없는 경우 대비)
            if not results and filter_conditions is not None:
                print("  카테고리 필터 결과 없음 → 필터 없이 재검색")
